...
const STORAGE_KEY = CONFIG.storageKey || "portfolio-dashboard-local-v1";
...
function loadInitialState() {
  ...
  return normalizeState({ holdings: [], trades: [], income: [], targets: clone(SAMPLE_DATA.targets) });
}
...
function normalizeState(raw) {
  return {
    ...
    targets: Array.isArray(raw.targets) ? raw.targets : clone(SAMPLE_DATA.targets)
  };
}
...
function label(value) {
  ...
  if (!value) return "-";
  return String(value).length <= 3 ? "•••" : `${String(value).slice(0, 1)}•••`;
}
...
function renderAppShell() {
  ...
  app.innerHTML = `
    <header class="topbar">
      <div>
        <div class="eyebrow">${CONFIG.modeLabel || "Sales Dashboard"}</div>
        <h1>${CONFIG.title || "투자 관리 대시보드"}</h1>
        <p class="subcopy">${CONFIG.description || "고객이 직접 입력한 투자 데이터를 정리하고 시각화하는 웹 대시보드입니다."}</p>
      </div>
...
function renderOverview() {
  const byAccount = sumBy(state.holdings, (row) => row.account, holdingValue);
  const byAsset = sumBy(state.holdings, (row) => row.assetClass, holdingValue);
  const latestIncome = Object.entries(sumBy(state.income, (row) => row.month, (row) => Number(row.gross || 0) - Number(row.tax || 0))).sort();
  return `
    <section class="grid-2">
      <article class="panel">
        <div class="panel-head"><h2>계좌별 평가금액</h2><span>3계좌 이상 고객용 핵심 화면</span></div>
        ${barList(byAccount)}
      </article>
      <article class="panel">
        <div class="panel-head"><h2>자산군 비중</h2><span>현재 포트폴리오 분포</span></div>
        ${barList(byAsset)}
      </article>
    </section>
    <section class="panel">
      <div class="panel-head"><h2>보유 종목 요약</h2><span>수동 입력/CSV 업로드 기반</span></div>
      ${holdingsTable(state.holdings.slice(0, 8))}
    </section>
    <section class="panel">
...
function renderAccounts() {
  const total = state.holdings.reduce((sum, row) => sum + holdingValue(row), 0);
  const currentByAsset = sumBy(state.holdings, (row) => row.assetClass, holdingValue);
  const rows = state.targets.map((target) => {
    const currentValue = currentByAsset[target.assetClass] || 0;
    const currentWeight = total ? (currentValue / total) * 100 : 0;
  ...
  return `
    <section class="panel">
      <div class="panel-head">
        <h2>장기투자 목표 비중 점검</h2>
        <span>현재 비중과 목표 비중의 괴리율 확인</span>
      </div>
    ...
    <section class="panel">
      <div class="panel-head"><h2>보유 종목 직접 추가</h2><span>고객 입력용 최소 폼</span></div>
      ${holdingForm()}
    </section>
  `;
}
...
function renderTrades() {
  ...
  return `
    ...
    <section class="panel">
      <div class="panel-head"><h2>스윙매매 복기</h2><span>종목 추천이 아닌 기록/복기 화면</span></div>
      <div class="table-wrap">
        <table>
    ...
    <section class="panel">
      <div class="panel-head"><h2>매매 기록 직접 추가</h2><span>진입·청산 사유를 남기는 구조</span></div>
      ${tradeForm()}
    </section>
  `;
}
...
function renderIncome() {
  ...
  return `
    ...
    <section class="panel">
      <div class="panel-head"><h2>현금흐름 직접 추가</h2><span>배당 또는 옵션 프리미엄</span></div>
      ${incomeForm()}
    </section>
  `;
}
...
function renderData() {
  return `
    <section class="grid-2">
      <article class="panel">
        <div class="panel-head"><h2>CSV 업로드</h2><span>헤더명이 일치해야 합니다</span></div>
        ${csvUploadBlock("holdings", "보유 종목 CSV", "account,ticker,name,assetClass,market,quantity,avgPrice,currentPrice")}
        ${csvUploadBlock("trades", "매매 복기 CSV", "entryDate,exitDate,ticker,strategy,entry,exit,quantity,entryReason,exitReason")}
        ${csvUploadBlock("income", "현금흐름 CSV", "month,account,ticker,type,gross,tax")}
      </article>
      <article class="panel">
    ...
    <section class="panel">
      <div class="panel-head"><h2>현재 저장 상태</h2><span>${STORAGE_KEY}</span></div>
...
function renderPrivacy() {
  return `
    <section class="panel prose">
      ...
      <h2>주의</h2>
      <p>고객이 화면 캡처, 백업 파일, CSV 파일을 운영자에게 직접 전달하는 경우 해당 내용은 운영자가 볼 수 있습니다. 따라서 “절대 볼 수 없음”, “100% 안전” 같은 표현은 사용하지 마세요.</p>
    </section>
  `;
}
...
function holdingForm() {
  return `
    <form id="holding-form" class="form-grid">
      <input name="account" placeholder="계좌명" required />
...
function incomeForm() {
  return `
    <form id="income-form" class="form-grid">
      ...
      <input name="account" placeholder="계좌명" />
...
function csvUploadBlock(type, title, headers) {
  return `
    <div class="csv-block">
      <h3>${title}</h3>
      <code>${headers}</code>
      <label class="secondary file-button">CSV 선택<input data-csv="${type}" type="file" accept=".csv,text/csv" /></label>
    </div>
  `;
}
  ...
  if (clearData) {
    clearData.addEventListener("click", () => {
      if (confirm("브라우저에 저장된 대시보드 데이터를 모두 삭제할까요?")) {
        state = normalizeState({ holdings: [], trades: [], income: [], targets: clone(SAMPLE_DATA.targets) });
        saveState();
        render();
      }
    });
...
function handleCsvImport(event) {
  ...
  reader.onload = () => {
    try {
      ...
      state[collection] = state[collection].concat(rows);
      saveState();
      render();
    } catch (error) {
...
function parseCsv(text) {
  ...
  return rows.slice(1).map((cells) => {
    ...
    headers.forEach((header, index) => {
      ...
      item[header] = value !== "" && Number.isFinite(asNumber) && !/^\d{4}-\d{2}(-\d{2})?$/.test(value) ? asNumber : value;
    });
    return item;
  });
}
